import numpy as np
import pandas as pd
import joblib, os, requests, pytz
...
#============================
#====CLASS NetLoadMLPModel=====
#============================
class NetLoadMLPModel:
    def __init__(self, upload_folder='NetLoad', model_filename='best_netload_model.joblib', data_filename='netload_resampled_and_merged.csv', scaler_filename="scaler_netload.joblib"):
        self.upload_folder = os.path.join(BASE_DIR, upload_folder)
        self.model_path = os.path.join(self.upload_folder, model_filename)
        self.data_path =  os.path.join(BASE_DIR, data_filename)
        os.makedirs(self.upload_folder, exist_ok=True)
        self.scaler = MinMaxScaler(feature_range=(0, 1))
        self.scaler_path =  os.path.join(self.upload_folder, scaler_filename)
        self.model = None
    ...
    def preprocess_test_data(self, df_weather, pv):
        ...
        #print("Shape df_weather:", df_weather.shape)
        #print(df_weather.head(5))
        #print("Shape df_pv:", df_pv.shape)
        #print(df_pv.head(5))
    ...
    def summarize_data(self, df_final):
    ...
    def run_pipeline(self, latitude, longitude, start_date, end_date):
        ...
        df_resampled= self.summarize_data(df_final)

        return df_resampled.to_string()
...
class OptimMLPModel():
    ...
    def __init__(self, forecast_dim='Production(W)', delta='1min'):
        pass
...
if __name__ == "__main__":
    ...
    #end_date = datetime(2024, 10, 31, 23, 59, 0)
    pred = mlp.run_pipeline(start_date, end_date, last_values=[100])
    print(pred)
    """
