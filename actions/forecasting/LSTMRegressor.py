...
class SolarLSTMModel:
    ...
    def train_or_load_model(self, df_data, month, year, epochs=50, batch_size=64):
        year = year - 1
        model_path = os.path.join(self.upload_folder, f"lstm_model_{month}-{year}_{self.forecast_dim.replace(' ', '_')}.pt")
        ...
        if os.path.exists(model_path):
            ...
            self.model = LSTMRegressor(input_size=input_size).to(self.device)
            self.model.load_state_dict(torch.load(model_path, map_location=self.device))
            """
            for s_path, attr in [(self.scaler_X_path, 'X'), (self.scaler_y_path, 'y')]:
                if os.path.exists(s_path):
                    setattr(self, f'scaler_{attr}', joblib.load(s_path))
            """
...
if __name__ == "__main__":
    lstm = SolarLSTMModel(upload_folder="Charge", data_filename="resampled_and_merged.csv", forecast_dim="Charge(W)", mode="direct")
    now = datetime.now()
    ...
    pred = lstm.run_pipeline(start_date, end_date, last_values=last_values_charge)
